  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      (!validateUsernamePassword(username) ||
        !validateUsernamePassword(password)) &&
      mode !== "forgot"
    ) {
    ...
    if (data.message.includes("successful") || data.message.includes("sent")) {
      // 成功後行為
      if (mode === "register") {
        alert(data.message);
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            <div className="relative my-5">
              <input
                ...
                className="input block w-72 py-2 pw-0 test-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-color-3 peer"
              />
            </div>
            {mode !== "forgot" && (
              <>
            ...
            <button
              type="submit"
              className="btn w-full mb-4 text-[18px] mt-6 rounded-full bg-white text-emerald-800 hover:bg-emerald-600 hover:text-white py-2 transition-colors duration-300"
            >
              {mode === "login"
                ? "Login"
                : mode === "register"
          ...
          <div className="flex justify-between">
            {(mode === "register" || mode === "forgot") && (
              <button
                className="btn btn-secondary m-2 hover:text-blue-400 duration-300"
